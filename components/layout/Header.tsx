...
function LanguageDropdown({ currentLocale, onSwitch }: { currentLocale: string; onSwitch: (lang: string) => void }) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <img src={currentLang?.flag} alt={currentLang?.label} width={18} height={13} className="rounded-sm object-cover" />
        {currentLang?.label}
          ...
          {languages.map((lang) => (
            <button
              ...
              className={`w-full flex items-center gap-2 px-3 py-2 text-xs transition-colors hover:bg-slate-100 dark:hover:bg-slate-800 ${
                currentLocale === lang.code ? "text-cyan-600 dark:text-cyan-400 font-semibold" : "text-slate-600 dark:text-slate-300"
              }`}
            >
              <img src={lang.flag} alt={lang.label} width={18} height={13} className="rounded-sm object-cover" />
              {lang.label}
            </button>
          ))}
        </div>
